...
def _read_metadata(metadata_file: Path = config.FAQ_METADATA_FILE) -> dict[str, Any]:
    """Read index metadata or return an empty mapping when unavailable."""
...
def _write_metadata(
    ...
    """Persist the document fingerprint and index creation timestamp."""
    payload = {
...
def ensure_index(
    embeddings: GoogleGenerativeAIEmbeddings | Any | None = None,
    *,
    ...
    """Load a local index or build one when the documents have changed."""
...
def create_faq_search_tool(retriever: QdrantRetriever) -> BaseTool:
    """Create the Qdrant-backed tool used by the next FAQ implementation."""

    @tool
    def qdrant_faq_search(query: str) -> str:
        ...
        return json.dumps({"resultados": evidences}, ensure_ascii=False)

    return qdrant_faq_search
